...
rep(
  ...
  '(0,l.useEffect)(()=>{if(!o)return;let t=e=>{let t=touchDrag;if(!t)return;let n=e.touches[0];if(!n)return;if(!t.started){if(Math.hypot(n.clientX-t.x0,n.clientY-t.y0)<12)return;t.started=!0,qe.current=!1,w(!0),E(t.kind===`pose`?{kind:`pose`,uid:t.uid}:{kind:`palette`,catalogueId:t.catalogueId})}e.preventDefault();let r=document.elementFromPoint(n.clientX,n.clientY)?.closest?.(`[data-slot-idx]`);r?x({rangee:+r.getAttribute(`data-rangee`),slot:+r.getAttribute(`data-slot-idx`)}):x(null)},n=e=>{let r=touchDrag,i=e.changedTouches[0];if(touchDrag=null,r&&!r.started){r.kind===`pose`&&O(r.uid);return}if(!i||!r?.started)return;e.preventDefault();let a=document.elementFromPoint(i.clientX,i.clientY)?.closest?.(`[data-slot-idx]`);if(a){let e=+a.getAttribute(`data-rangee`),t=+a.getAttribute(`data-slot-idx`),n=ie(),i=r.kind===`pose`?r.uid:void 0;ae(e,t,n,i)&&(qe.current=!0,r.kind===`pose`?F(r.uid,e,t):r.kind===`palette`&&P(e,t,r.catalogueId))}D(!0)};return window.addEventListener(`touchmove`,t,{capture:!0,passive:!1}),window.addEventListener(`touchend`,n,{capture:!0,passive:!1}),window.addEventListener(`touchcancel`,n,{capture:!0,passive:!1}),()=>{window.removeEventListener(`touchmove`,t,{capture:!0}),window.removeEventListener(`touchend`,n,{capture:!0}),window.removeEventListener(`touchcancel`,n,{capture:!0})}},[o]),',
...
rep(
  ...
  'onTouchStart:e=>{if(e.target.closest(`.composant-suppr`))return;let t=e.touches[0];t&&(touchDrag={kind:`pose`,uid:s.uid,x0:t.clientX,y0:t.clientY,started:!1})},onDragStart:e=>{qe.current=!1,Gt(e.currentTarget,e)',
  'composant onTouchStart drag'
);

rep(
  'className:`palette-item`,draggable:!0,onDragStart:n=>{n.dataTransfer.setData(`text/plain`,t.id)',
  'className:`palette-item`,draggable:!0,onTouchStart:n=>{let e=n.touches[0];e&&(touchDrag={kind:`palette`,catalogueId:t.id,x0:e.clientX,y0:e.clientY,started:!1})},onDragStart:n=>{n.dataTransfer.setData(`text/plain`,t.id)',
